...
医疗公司业绩预告/快报用 4+1 维度（标准4维度 + 医疗业务亮点）。
医疗公司年报/半年报用 5 维度（研发费用/分产品收入/现金储备/授权合作/监管里程碑）。
...
# ===== 医疗专属业绩预告/快报 prompt (4+1维度) =====
# 业绩预告/快报 PDF 通常只有1-5页，不含研发费用/管线进展等详细数据
# 在标准4维度基础上增加"医疗业务亮点"维度，捕捉公告中提及的医疗相关信号
MEDICAL_FORECAST_PROMPT_TEMPLATE = """你是医药行业财务公告解析助手。基于以下公告文本，提取 5 个维度的信息。

铁律:
1. 文本中没有的数据写"未披露"，不许编造。
2. 数字必须原文照录，不做换算。
3. 简洁直白，不写废话。
4. 重点关注医疗业务相关信号（管线、订单、技术平台、客户拓展等）。
...
## 医疗业务亮点
- 管线与产品: （如有提及新药研发、临床推进、产品上市、管线推进等）
- 订单与客户: （如有提及新签订单、客户拓展、订单增长等）
- 技术平台: （如有提及技术平台建设、新疗法布局、产能扩张等）
- 监管与合作: （如有提及IND/NDA/BLA、license-in/out、BD合作等）
（公告中未提及相关内容则写"未提及"）
"""
...
def parse_finreport(company: str, report_type: str, title: str, period: str,
    ...
    try:
        ...
        # 选择 prompt 模板：
        # - 非医疗 → 标准 4 维度
        # - 医疗 + 业绩预告/快报 → 轻量医疗 4+1 维度（业绩预告太短，不含研发/管线详情）
        # - 医疗 + 年报/半年报/季报 → 完整医疗 5 维度
        _FORECAST_TYPES = {"业绩预告", "业绩快报", "Profit Warning", "Profit Alert"}
        if is_medical and report_type in _FORECAST_TYPES:
            template = MEDICAL_FORECAST_PROMPT_TEMPLATE
        elif is_medical:
...
def run_finreport_dossiers(reports: list, out_dir: Path,
    ...
    for r in targets:
        ...
        if md:
            med_tag = "【医疗拆解】" if is_med else "【财报拆解】"
            full_md = (f"# {med_tag}{r.company_name} ({r.stock_code})\n\n"
